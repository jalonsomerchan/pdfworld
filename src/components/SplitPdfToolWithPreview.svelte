<script lang="ts">
  ...
  type Lang = 'es' | 'en';
  type ParsedRange = { label: string; pages: number[] };
  ...
  const labels = {
    es: {
      ...
      description: 'Carga un PDF, revisa miniaturas de sus páginas y descarga una selección como PDF único o como ZIP con PDFs separados.',
      ...
      pages: 'páginas',
      page: 'Página',
      thumb: 'Miniatura de página',
      pageCountLabel: 'Páginas detectadas',
      selectedFile: 'Archivo seleccionado',
      selectorTitle: 'Selecciona las páginas',
      selectorHelp: 'Pulsa sobre cada miniatura para incluirla o quitarla. El orden de exportación será el orden original del PDF.',
    ...
    en: {
      ...
      description: 'Load a PDF, review page thumbnails and download a selection as one PDF or as a ZIP with separate PDFs.',
      ...
      selectorHelp: 'Click each thumbnail to include it or remove it. Export order follows the original PDF order.',
  ...
  $: selectionLabel = selectedPages.join(', ');
  ...
  async function loadFile(nextFile: File | null) {
    ...
    try {
      ...
      rangeInput = parsedRanges[0]?.label ?? '';
      thumbStatuses = Object.fromEntries(selectedPages.map((page) => [page, 'pending']));
  ...
  async function renderThumbnails(bytes: Uint8Array) {
    ...
    try {
      const task = getDocument({ data: bytes.slice(), useWorkerFetch: false, isEvalSupported: false, disableAutoFetch: true, disableStream: true });
      ...
      for (let pageNumber = 1; pageNumber <= previewDocument.numPages; pageNumber += 1) {
        ...
        try {
          ...
          const url = await renderPageThumbnail(pdfPage);
          pdfPage.cleanup();
          ...
          if (thumbUrls[pageNumber]) URL.revokeObjectURL(thumbUrls[pageNumber]);
          thumbUrls = { ...thumbUrls, [pageNumber]: url };
          thumbStatuses = { ...thumbStatuses, [pageNumber]: 'ready' };
        } catch {
          thumbStatuses = { ...thumbStatuses, [pageNumber]: 'failed' };
        }
      }
    } catch {
  ...
  function togglePage(page: number) {
    ...
    selectedPages = selectedPages.includes(page)
  ...
  function parseRanges(input: string, totalPages: number) {
    ...
    if (!value) return { pages, ranges, error: '' };
    if (!totalPages) return { pages, ranges, error: t.needFile };
    ...
    for (const chunk of chunks) {
      ...
      for (const page of rangePages) {
        if (page > totalPages) return { pages: [], ranges: [], error: t.outOfBounds.replace('{page}', String(page)).replace('{total}', String(totalPages)) };
  ...
  function createRange(start: number, end: number): ParsedRange {
    return { label: start === end ? String(start) : `${start}-${end}`, pages: Array.from({ length: end - start + 1 }, (_, index) => start + index) };
  }
...
<section class="split-tool" aria-labelledby="split-tool-title">
  ...
  {#if isLoading}
    <div class="split-tool__alert" role="status">{t.loading}</div>
  {/if}

  {#if isRendering}
    <div class="split-tool__alert" role="status">{t.rendering}</div>
  {/if}
      ...
      <section class="split-tool__selector-card" aria-labelledby="split-selector-title">
        ...
        <div class="split-tool__page-grid" aria-label={t.selectorTitle}>
          {#each pageNumbers as page}
            ...
            >
              <span class="split-tool__page-thumb" aria-hidden={thumbStatuses[page] !== 'ready'}>
                {#if thumbStatuses[page] === 'ready' && thumbUrls[page]}
                  <img src={thumbUrls[page]} alt={`${t.thumb} ${page}`} loading="lazy" />
                {:else if thumbStatuses[page] === 'failed'}
                  <span class="split-tool__page-thumb-placeholder">{page}</span>
                {:else}
                  <span class="split-tool__page-thumb-placeholder split-tool__page-thumb-placeholder--loading">{page}</span>
                {/if}
              </span>
              <span class="split-tool__page-meta">
                <span>{t.page}</span>
                <strong>{page}</strong>
              </span>
            </button>
...
<style>
  .split-tool { display: grid; gap: 20px; margin: 32px 0 64px; }
  .split-tool__upload-panel, .split-tool__workspace, .split-tool__file-card, .split-tool__selector-card, .split-tool__form-card, .split-tool__alert { border: 1px solid #e2e8f0; border-radius: 24px; background: rgba(255,255,255,.86); box-shadow: 0 24px 70px rgba(15,23,42,.08); }
  .split-tool__upload-panel { position: relative; display: grid; grid-template-columns: minmax(0,1.05fr) minmax(300px,.95fr); gap: 20px; align-items: center; overflow: hidden; padding: 24px; border-radius: 32px; background: radial-gradient(circle at top right, rgba(239,68,68,.14), transparent 36%), linear-gradient(135deg, rgba(255,255,255,.96), rgba(248,250,252,.88)); box-shadow: 0 30px 90px rgba(15,23,42,.11); }
  .split-tool__upload-panel::before { content: ''; position: absolute; inset: 0; pointer-events: none; background: linear-gradient(120deg, transparent 20%, rgba(255,255,255,.62), transparent 80%); transform: translateX(-110%); animation: split-upload-sheen 7s ease-in-out infinite; }
  .split-tool__upload-panel > * { position: relative; z-index: 1; }
  .split-tool__intro h2 { margin: 0 0 8px; letter-spacing: -.04em; }
  .split-tool__intro p, .split-tool__file-card p, .split-tool__field p, .split-tool__preview p, .split-tool__selector-header p { margin: 0; color: #64748b; }
  .split-tool__eyebrow { display: inline-flex; margin-bottom: 10px; padding: 6px 10px; border-radius: 999px; background: #fee2e2; color: #991b1b; font-size: .8rem; font-weight: 900; }
  .split-tool__alert { padding: 16px 18px; color: #475569; font-weight: 850; }
  .split-tool__alert--error { border-color: #fecaca; background: #fff1f2; color: #991b1b; }
  .split-tool__alert--success { border-color: #bbf7d0; background: #f0fdf4; color: #166534; }
  .split-tool__workspace { display: grid; grid-template-columns: minmax(220px,.72fr) minmax(0,1.28fr); gap: 18px; padding: 18px; }
  .split-tool__file-card, .split-tool__selector-card, .split-tool__form-card { padding: 18px; }
  .split-tool__selector-card, .split-tool__form-card { grid-column: 1 / -1; }
  .split-tool__file-card { display: flex; gap: 14px; align-items: flex-start; }
  .split-tool__file-card h3, .split-tool__preview h3, .split-tool__selector-card h3 { margin: 0 0 8px; }
  .split-tool__file-card strong { display: block; overflow: hidden; max-width: 100%; text-overflow: ellipsis; white-space: nowrap; }
  .split-tool__file-icon { display: grid; width: 52px; height: 64px; place-items: center; border-radius: 16px; background: #fff1f2; color: #dc2626; font-size: .75rem; font-weight: 950; box-shadow: inset 0 0 0 1px rgba(239,68,68,.14); }
  .split-tool__selector-card, .split-tool__form-card { display: grid; gap: 18px; }
  .split-tool__selector-header { display: flex; justify-content: space-between; gap: 14px; align-items: flex-start; }
  .split-tool__selector-header strong { flex: 0 0 auto; padding: 8px 11px; border-radius: 999px; background: #fef3c7; color: #92400e; font-size: .88rem; }
  .split-tool__quick-actions, .split-tool__actions { display: flex; flex-wrap: wrap; gap: 10px; }
  .split-tool__quick-actions button, .split-tool__primary, .split-tool__secondary, .split-tool__page-button { border: 0; cursor: pointer; font: inherit; font-weight: 950; transition: transform 140ms ease, box-shadow 140ms ease, background 140ms ease, border-color 140ms ease; }
  .split-tool__quick-actions button { min-height: 40px; padding: 9px 13px; border-radius: 999px; background: #e2e8f0; color: #334155; }
  .split-tool__page-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(124px, 1fr)); gap: 12px; max-height: 560px; overflow: auto; padding: 4px; }
  .split-tool__page-button { display: grid; align-content: start; gap: 10px; min-height: 184px; padding: 10px; border: 2px solid #e2e8f0; border-radius: 18px; background: #fff; color: #0f172a; text-align: left; box-shadow: 0 10px 24px rgba(15,23,42,.05); }
  .split-tool__page-thumb { display: grid; min-height: 138px; place-items: center; overflow: hidden; border: 1px solid #e2e8f0; border-radius: 13px; background: #f8fafc; }
  .split-tool__page-thumb img { display: block; width: 100%; height: auto; pointer-events: none; }
  .split-tool__page-thumb-placeholder { display: grid; width: 100%; min-height: 138px; place-items: center; color: #94a3b8; font-size: 1.25rem; font-weight: 950; }
  .split-tool__page-thumb-placeholder--loading { background: linear-gradient(100deg, #f1f5f9 20%, #fff 45%, #f1f5f9 70%); background-size: 220% 100%; animation: split-thumb-loading 1.4s ease-in-out infinite; }
  .split-tool__page-meta { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
  .split-tool__page-meta span { color: #64748b; font-size: .76rem; text-transform: uppercase; }
  .split-tool__page-meta strong { font-size: 1.08rem; }
  .split-tool__page-button--selected { border-color: #ef4444; background: #fff1f2; color: #991b1b; box-shadow: 0 16px 34px rgba(239,68,68,.16); }
  .split-tool__page-button:hover, .split-tool__quick-actions button:hover:not(:disabled), .split-tool__primary:hover:not(:disabled), .split-tool__secondary:hover:not(:disabled) { transform: translateY(-1px); }
  .split-tool__field { display: grid; gap: 8px; }
  .split-tool__field label { color: #0f172a; font-weight: 950; }
  .split-tool__field input { width: 100%; min-height: 52px; box-sizing: border-box; border: 2px solid #e2e8f0; border-radius: 18px; background: #fff; color: #0f172a; font: inherit; font-size: 1.08rem; font-weight: 850; padding: 12px 14px; transition: border-color 160ms ease, box-shadow 160ms ease; }
  .split-tool__field input:focus { border-color: #ef4444; box-shadow: 0 0 0 4px rgba(239,68,68,.12); outline: none; }
  .split-tool__field input[aria-invalid='true'] { border-color: #ef4444; }
  .split-tool__preview { display: grid; gap: 10px; padding: 14px; border-radius: 18px; background: #f8fafc; }
  .split-tool__chips { display: flex; flex-wrap: wrap; gap: 8px; }
  .split-tool__chips span { display: inline-flex; padding: 7px 10px; border-radius: 999px; background: #fee2e2; color: #991b1b; font-weight: 950; }
  .split-tool__actions { justify-content: flex-end; }
  .split-tool__primary, .split-tool__secondary { min-height: 46px; padding: 12px 16px; border-radius: 999px; }
  .split-tool__primary { background: linear-gradient(135deg, #ef4444, #b91c1c); color: #fff; box-shadow: 0 16px 34px rgba(239,68,68,.24); }
  .split-tool__primary--dark { background: linear-gradient(135deg, #0f172a, #334155); box-shadow: 0 16px 34px rgba(15,23,42,.22); }
  .split-tool__secondary { background: #e2e8f0; color: #334155; }
  .split-tool__primary:disabled, .split-tool__secondary:disabled { cursor: not-allowed; opacity: .45; }
  @keyframes split-upload-sheen { 0%, 55% { transform: translateX(-110%); } 100% { transform: translateX(110%); } }
  @keyframes split-thumb-loading { 0% { background-position: 120% 0; } 100% { background-position: -120% 0; } }
  @media (prefers-reduced-motion: reduce) { .split-tool__upload-panel::before, .split-tool__page-thumb-placeholder--loading { animation: none; } .split-tool__field input, .split-tool__quick-actions button, .split-tool__primary, .split-tool__secondary, .split-tool__page-button { transition: none; } }
  @media (max-width: 760px) { .split-tool__upload-panel, .split-tool__workspace { grid-template-columns: 1fr; } .split-tool__upload-panel { padding: 18px; border-radius: 24px; } .split-tool__selector-header { flex-direction: column; } .split-tool__page-grid { grid-template-columns: repeat(auto-fill, minmax(104px, 1fr)); max-height: 460px; } .split-tool__page-button { min-height: 166px; } .split-tool__page-thumb, .split-tool__page-thumb-placeholder { min-height: 116px; } .split-tool__actions { justify-content: stretch; } .split-tool__primary, .split-tool__secondary { flex: 1 1 180px; } }
</style>
