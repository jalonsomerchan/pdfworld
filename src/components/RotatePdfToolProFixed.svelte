<script lang="ts">
  ...
  const i18n = {
    es: {
      ...
      optionsBadge: 'Opciones', optionsTitle: 'Opciones de rotación', optionsDescription: 'Aplica giros rápidos, revisa los cambios y genera una copia nueva del PDF sin subir archivos.',
    },
    en: {
      ...
      optionsBadge: 'Options', optionsTitle: 'Rotation options', optionsDescription: 'Apply quick rotations, review changes and generate a new PDF copy without uploads.',
    },
  } as const;
  ...
  $: optionActions = [
    { label: t.clear, variant: 'secondary', disabled: isExporting, onClick: clearTool },
    { label: isExporting ? t.generating : t.generate, variant: 'primary', disabled: !canGenerate, onClick: generatePreview },
    { label: t.download, variant: 'secondary', disabled: !canDownload, onClick: () => (isPreviewOpen = true) },
      ...
      <aside class="rotate-pro__sidebar">
        ...
        >
          <section class="tool-options-group">
            <h3 class="tool-options-group__title">{t.quick}</h3>
          ...
          <section class="tool-options-group" aria-live="polite">
            <h3 class="tool-options-group__title">{t.summary}</h3>
            {#if changedStates.length}
              <p><strong>{changedStates.length}</strong> {t.changed}</p>
              <div class="rotate-pro__summary-list">
                {#each changedStates as item}
                  <span>{t.page} {item.page}: {item.kept ? `${item.rotation}°` : t.removePage}</span>
                {/each}
              </div>
            {:else}
              <p class="tool-options-help">{t.noChanges}</p>
            {/if}
          </section>

          <section class="tool-options-group rotate-pro__metrics">
