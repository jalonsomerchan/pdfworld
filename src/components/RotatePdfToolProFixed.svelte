<script lang="ts">
  ...
  const i18n = {
    es: {
      ...
      rotateLeft: 'Girar izquierda', rotateRight: 'Girar derecha', resetPage: 'Restablecer página', summary: 'Resumen', noChanges: 'Gira una o varias páginas para activar la vista previa.', changed: 'páginas modificadas',
      ...
      ready: 'PDF rotado correctamente. Revisa la vista previa antes de descargarlo.', needFile: 'Primero carga un PDF.', needChanges: 'Gira al menos una página antes de generar el PDF.', createError: 'No se pudo rotar el PDF. Revisa que el archivo no esté protegido o dañado.', outputName: 'pdfworld-rotado.pdf',
    },
    en: {
      ...
      rotateLeft: 'Rotate left', rotateRight: 'Rotate right', resetPage: 'Reset page', summary: 'Summary', noChanges: 'Rotate one or more pages to enable the preview.', changed: 'pages changed',
      ...
      ready: 'PDF rotated successfully. Review the preview before downloading it.', needFile: 'Load a PDF first.', needChanges: 'Rotate at least one page before generating the PDF.', createError: 'The PDF could not be rotated. Check that the file is not protected or damaged.', outputName: 'pdfworld-rotated.pdf',
    },
  } as const;
  ...
  $: canGenerate = Boolean(file && changedStates.length > 0 && !isLoading && !isExporting);
  ...
  async function loadFile(nextFile: File | null) {
    ...
    file = nextFile; pageCount = 0; rotations = {}; thumbUrls = {}; thumbStatus = {}; result = null; isPreviewOpen = false; errorMessage = ''; statusMessage = '';
    if (!nextFile) return;
    isLoading = true;
    try {
  ...
  async function generatePreview() {
    ...
    try {
      ...
      result = nextResult;
      previewUrl = URL.createObjectURL(new Blob([new Uint8Array(nextResult.bytes)], { type: 'application/pdf' }));
      isPreviewOpen = true;
  ...
  function downloadPdf() { if (result) saveBytes(result.bytes); }
  ...
  function clearTool() {
    ...
    file = null; pageCount = 0; rotations = {}; thumbUrls = {}; thumbStatus = {}; result = null; isPreviewOpen = false; isLoading = false; isRendering = false; isExporting = false; errorMessage = ''; statusMessage = '';
  }
  ...
  {#if file && pageCount > 0}
    <div class="rotate-pro__workspace">
      <aside class="rotate-pro__sidebar">
        ...
        <div class="rotate-pro__actions"><button type="button" class="rotate-pro__secondary" on:click={clearTool} disabled={isExporting}>{t.clear}</button><button type="button" class="rotate-pro__primary" on:click={generatePreview} disabled={!canGenerate}>{isExporting ? t.generating : t.generate}</button><button type="button" class="rotate-pro__primary rotate-pro__primary--dark" on:click={downloadPdf} disabled={!canDownload}>{t.download}</button></div>
      </aside>
...
{#if isPreviewOpen && previewUrl}
  <div class="pdf-modal" role="dialog" aria-modal="true" aria-labelledby="pdf-modal-title">
    ...
    <div class="pdf-modal__panel">
      <header class="pdf-modal__header"><h2 id="pdf-modal-title">{t.previewTitle}</h2><div><button type="button" class="pdf-modal__download" on:click={downloadPdf}>{t.download}</button><button type="button" class="pdf-modal__close" on:click={closePreview} aria-label={t.closePreview}>×</button></div></header>
      <object class="pdf-modal__viewer" data={previewUrl} type="application/pdf"><p>{t.previewFallback}</p><button type="button" class="pdf-modal__download" on:click={downloadPdf}>{t.download}</button></object>
    </div>
  </div>
{/if}
