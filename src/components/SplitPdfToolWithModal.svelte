<script lang="ts">
  ...
  const labels = {
    es: {
      ...
      selectedFile: 'Archivo seleccionado', pages: 'páginas', page: 'Página', selected: 'páginas seleccionadas', selectAll: 'Seleccionar todas', selectNone: 'Quitar selección', invert: 'Invertir selección',
    ...
    en: {
      ...
      selectedFile: 'Selected file', pages: 'pages', page: 'Page', selected: 'selected pages', selectAll: 'Select all', selectNone: 'Clear selection', invert: 'Invert selection',
  ...
  async function loadFile(nextFile: File | null) {
    ...
    file = nextFile; sourceBytes = null; pageCount = 0; selectedPages = []; ranges = []; rangeInput = ''; thumbUrls = {}; thumbStatus = {}; errorMessage = ''; statusMessage = '';
    if (!nextFile) return;
    isLoading = true;
    try {
  ...
  function downloadPdf() { if (resultBytes) saveAs(new Blob([new Uint8Array(resultBytes)], { type: 'application/pdf' }), getSingleOutputFilename(selectedPages)); }
...
<section class="split-modal-tool" aria-labelledby="split-tool-title">
  ...
  {#if file && pageCount > 0}
    <div class="split-modal-tool__workspace">
      ...
      <section class="split-modal-tool__pages" aria-label={t.title}>
        ...
        <div class="split-modal-tool__grid">
          {#each pageNumbers as page}
            <button type="button" class:split-modal-tool__page={true} class:split-modal-tool__page--selected={selectedPages.includes(page)} aria-pressed={selectedPages.includes(page)} on:click={() => togglePage(page)}>
              <span class="split-modal-tool__thumb">{#if thumbStatus[page] === 'ready' && thumbUrls[page]}<img src={thumbUrls[page]} alt={`${t.page} ${page}`} loading="lazy" />{:else}<span class:split-modal-tool__placeholder={true} class:split-modal-tool__placeholder--loading={thumbStatus[page] === 'pending'}>{page}</span>{/if}</span>
              <span><small>{t.page}</small><strong>{page}</strong></span>
            </button>
          {/each}
        </div>
      </section>
...
{#if isPreviewOpen && previewUrl}
  <div class="pdf-modal" role="dialog" aria-modal="true" aria-labelledby="split-preview-title">
    <div class="pdf-modal__backdrop" on:click={closePreview} aria-hidden="true"></div>
    <div class="pdf-modal__panel">
      <header class="pdf-modal__header"><h2 id="split-preview-title">{t.previewTitle}</h2><div><button type="button" class="pdf-modal__download" on:click={downloadPdf}>{t.download}</button><button type="button" class="pdf-modal__close" on:click={closePreview} aria-label={t.closePreview}>×</button></div></header>
