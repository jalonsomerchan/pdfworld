<script lang="ts">
  ...
  const labels = {
    es: {
      ...
      clear: 'Limpiar', generatePreview: 'Generar vista previa', generating: 'Generando PDF…', download: 'Descargar PDF', rendering: 'Generando miniaturas… puedes editar mientras tanto.', downloadName: 'pdfworld-unido.pdf',
    ...
    en: {
      ...
      clear: 'Clear', generatePreview: 'Generate preview', generating: 'Generating PDF…', download: 'Download PDF', rendering: 'Generating thumbnails… you can edit meanwhile.', downloadName: 'pdfworld-merged.pdf',
  ...
  function downloadPdf() { if (resultBytes) saveAs(new Blob([new Uint8Array(resultBytes)], { type: 'application/pdf' }), t.downloadName); }
...
<section class="merge-pro" aria-labelledby="merge-tool-title">
  <div class="merge-pro__hero">
    <div><span class="merge-pro__eyebrow">PDFWorld</span><h2 id="merge-tool-title">{t.title}</h2><p>{t.description}</p></div>
      ...
      <aside class="merge-pro__sources" aria-labelledby="sources-title">
        ...
        {#each sources as source (source.id)}
          <article draggable="true" on:dragstart={(event) => onSourceDragStart(event, source.id)} on:dragover={(event) => onSourceDragOver(event, source.id)} on:drop={(event) => onSourceDrop(event, source.id)} on:dragend={() => (dragOverSourceId = '')} class:merge-pro__source={true} class:merge-pro__source--error={Boolean(source.error)} class:merge-pro__source--over={dragOverSourceId === source.id}>
            <div class="merge-pro__source-main"><span class="merge-pro__drag-handle" aria-hidden="true">⋮⋮</span><span class="merge-pro__file-icon">PDF</span><div><strong>{source.file.name}</strong><p>{formatSize(source.file.size)} · {sourcePages(source.id) || source.pageCount} {t.pages}</p>{#if source.error}<p class="merge-pro__file-error">{source.error}</p>{/if}</div></div>
        ...
        <button type="button" class="merge-pro__primary merge-pro__primary--dark" disabled={!canDownload} on:click={downloadPdf}>{t.download}</button>
      </aside>
...
{#if isPreviewOpen && previewUrl}
  <div class="pdf-modal" role="dialog" aria-modal="true" aria-labelledby="merge-preview-title">
    <div class="pdf-modal__backdrop" on:click={closePreview} aria-hidden="true"></div>
    <div class="pdf-modal__panel">
      <header class="pdf-modal__header"><h2 id="merge-preview-title">{t.previewTitle}</h2><div><button type="button" class="pdf-modal__download" on:click={downloadPdf}>{t.download}</button><button type="button" class="pdf-modal__close" on:click={closePreview} aria-label={t.closePreview}>×</button></div></header>
