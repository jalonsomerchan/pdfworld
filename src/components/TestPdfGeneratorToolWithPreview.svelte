<script lang="ts">
  ...
  const l = {
    es: {
      ...
      generate: 'Generar vista previa', generating: 'Generando PDF…', reset: 'Restablecer configuración', downloadName: 'pdfworld-pdf-prueba', ready: 'PDF generado correctamente. Revisa la vista previa antes de descargar.', error: 'No se pudo generar el PDF. Revisa la configuración o reduce páginas/imágenes.',
      previewTitle: 'Vista previa del PDF de prueba', previewDescription: 'Comprueba el documento generado y descárgalo cuando esté listo.', closePreview: 'Cerrar vista previa', download: 'Descargar PDF', openPreview: 'Abrir en pestaña',
    ...
    en: {
      ...
      generate: 'Generate preview', generating: 'Generating PDF…', reset: 'Reset configuration', downloadName: 'pdfworld-test-pdf', ready: 'PDF generated successfully. Review the preview before downloading.', error: 'The PDF could not be generated. Check settings or reduce pages/images.',
      previewTitle: 'Test PDF preview', previewDescription: 'Check the generated document and download it when it is ready.', closePreview: 'Close preview', download: 'Download PDF', openPreview: 'Open in tab',
  ...
  const sizes = { a4: [595.28, 841.89], a3: [841.89, 1190.55], a5: [419.53, 595.28], letter: [612, 792], legal: [612, 1008] } as const;
  ...
  function applyPreset(preset: 'small' | 'medium' | 'large' | 'images' | 'text') {
    markDirty();
    if (preset === 'small') { pageCount = 1; density = 'low'; imageMode = 'none'; imagePlacement = 'text'; pagePreset = 'a4'; }
    if (preset === 'medium') { pageCount = 10; density = 'medium'; imageMode = 'placeholder'; imagePlacement = 'top'; imageScope = 'odd'; pagePreset = 'a4'; }
    if (preset === 'large') { pageCount = 100; density = 'medium'; imageMode = 'none'; imagePlacement = 'text'; pagePreset = 'a4'; }
    if (preset === 'images') { pageCount = 25; density = 'low'; imageMode = 'placeholder'; imagePlacement = 'bottom'; imageScope = 'all'; pagePreset = 'a4'; }
    if (preset === 'text') { pageCount = 20; density = 'high'; imageMode = 'none'; imagePlacement = 'text'; pagePreset = 'letter'; }
  }

  function resetConfig() {
    pageCount = 10; documentTitle = 'PDF de prueba FácilPDF'; author = 'FácilPDF'; subject = 'Documento de prueba generado localmente'; keywords = 'pdf, prueba, ejemplo, facilpdf'; addPageNumbers = true; pagePreset = 'a4'; orientation = 'portrait'; customWidth = 210; customHeight = 297; customUnit = 'mm'; density = 'medium'; customWordsPerPage = 250; fontSize = 12; marginMm = 18; imageMode = 'none'; imagePlacement = 'text'; imageScope = 'all'; imagePages = '1-3'; uploadedImage = null; uploadedImageName = ''; errorMessage = ''; statusMessage = ''; clearPreview();
  }
  ...
  async function generatePdf() {
    ...
    try {
      ...
      for (let pageIndex = 0; pageIndex < pageCount; pageIndex += 1) {
        ...
        const hasImage = Boolean(embeddedImage && imagePagesSet.has(pageNumber));
        ...
        const text = bodyText.split(' ').slice(0, wordsPerPage).join(' ');
        drawWrappedText({ page, text, font, fontSize, margin, width, startY: cursorY, imageBox: imagePlacement === 'wrap' && hasImage && imageBox ? imageBox : null });

        if (hasImage && embeddedImage && imageBox && (imagePlacement === 'bottom' || imagePlacement === 'wrap')) {
          drawImage(page, embeddedImage, imageBox);
        }
      ...
      resultFilename = `${t.downloadName}-${pageCount}-paginas.pdf`;
      previewUrl = URL.createObjectURL(new Blob([new Uint8Array(bytes)], { type: 'application/pdf' }));
      isPreviewOpen = true;
  ...
  function clearPreview() { if (previewUrl) URL.revokeObjectURL(previewUrl); previewUrl = ''; resultFilename = ''; isPreviewOpen = false; }

  function drawWrappedText(args: { page: any; text: string; font: PDFFont; fontSize: number; margin: number; width: number; startY: number; imageBox: { x: number; y: number; width: number; height: number } | null }) {
    ...
    if (line && y >= bottomY) page.drawText(line, { x: margin, y, size: fontSize, font, color: rgb(0.24, 0.26, 0.32), lineHeight });
  }
  ...
  function getImageBox(width: number, height: number, margin: number, cursorY: number, ratio: number) {
    if (imagePlacement === 'wrap') {
      ...
      return { x: width - margin - boxWidth, y: cursorY - boxWidth / ratio, width: boxWidth, height: boxWidth / ratio };
    }
    const boxWidth = Math.min(width - margin * 2, 360);
    const boxHeight = Math.min(boxWidth / ratio, height * 0.24);
    return { x: margin + (width - margin * 2 - boxWidth) / 2, y: imagePlacement === 'bottom' ? margin + 52 : cursorY - boxHeight, width: boxWidth, height: boxHeight };
  }
  ...
  async function createPlaceholderImage() {
    ...
    gradient.addColorStop(0, '#fee2e2'); gradient.addColorStop(1, '#dbeafe');
    ctx.fillStyle = gradient; ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.fillStyle = '#991b1b'; ctx.font = 'bold 72px system-ui, sans-serif'; ctx.fillText('FácilPDF', 70, 150);
    ctx.fillStyle = '#334155'; ctx.font = '36px system-ui, sans-serif'; ctx.fillText('Imagen de prueba generada localmente', 70, 230);
    ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 12; ctx.strokeRect(40, 40, canvas.width - 80, canvas.height - 80);
  ...
  function parseRanges(input: string, total: number) {
    const pages: number[] = [];
    if (!input.trim()) return { pages, error: 'empty' };
  ...
  function buildLoremWords(words: number) { const source = lorem.split(' '); return Array.from({ length: words }, (_, index) => source[index % source.length]).join(' '); }
  function mmToPt(mm: number) { return mm * 72 / 25.4; }
  function formatPageSize() { return `${Math.round(pageSize[0])} × ${Math.round(pageSize[1])} pt`; }
  function yieldToBrowser() { return new Promise<void>((resolve) => window.setTimeout(resolve, 0)); }
</script>

<section class="test-pdf" aria-labelledby="test-pdf-title">
  <div class="test-pdf__hero"><div><span class="test-pdf__eyebrow">FácilPDF</span><h2 id="test-pdf-title">{t.title}</h2><p>{t.intro}</p></div><div class="test-pdf__summary" aria-live="polite"><strong>{pageCount}</strong><span>{t.pages}</span><small>{estimatedCharacters.toLocaleString()} chars · ~{estimatedSizeKb} KB</small></div></div>
  <div class="test-pdf__presets" aria-label="Presets rápidos"><button type="button" on:click={() => applyPreset('small')}>{t.presetSmall}</button><button type="button" on:click={() => applyPreset('medium')}>{t.presetMedium}</button><button type="button" on:click={() => applyPreset('large')}>{t.presetLarge}</button><button type="button" on:click={() => applyPreset('images')}>{t.presetImages}</button><button type="button" on:click={() => applyPreset('text')}>{t.presetText}</button></div>
  <div class="test-pdf__grid">
    <fieldset class="test-pdf__panel"><legend>{t.doc}</legend><label>{t.pages}<input type="number" min="1" max="120" bind:value={pageCount} on:input={markDirty} /></label><label>{t.titleField}<input bind:value={documentTitle} on:input={markDirty} /></label><label>{t.author}<input bind:value={author} on:input={markDirty} /></label><label>{t.subject}<input bind:value={subject} on:input={markDirty} /></label><label>{t.keywords}<input bind:value={keywords} on:input={markDirty} /></label><label class="test-pdf__check"><input type="checkbox" bind:checked={addPageNumbers} on:change={markDirty} /> {t.numbering}</label></fieldset>
    <fieldset class="test-pdf__panel"><legend>{t.page}</legend><label>{t.pageSize}<select bind:value={pagePreset} on:change={markDirty}><option value="a4">A4</option><option value="a3">A3</option><option value="a5">A5</option><option value="letter">Letter</option><option value="legal">Legal</option><option value="custom">{t.customSize}</option></select></label><label>{t.orientation}<select bind:value={orientation} on:change={markDirty}><option value="portrait">{t.portrait}</option><option value="landscape">{t.landscape}</option></select></label>{#if pagePreset === 'custom'}<div class="test-pdf__inline"><label>{t.width}<input type="number" min="1" bind:value={customWidth} on:input={markDirty} /></label><label>{t.height}<input type="number" min="1" bind:value={customHeight} on:input={markDirty} /></label><label>{t.unit}<select bind:value={customUnit} on:change={markDirty}><option value="mm">mm</option><option value="px">px</option></select></label></div>{/if}<label>{t.margins}<input type="number" min="0" max="80" bind:value={marginMm} on:input={markDirty} /></label><p class="test-pdf__help">{t.marginHelp}</p></fieldset>
    <fieldset class="test-pdf__panel"><legend>{t.content}</legend><label>{t.density}<select bind:value={density} on:change={markDirty}><option value="low">{t.low}</option><option value="medium">{t.medium}</option><option value="high">{t.high}</option><option value="custom">{t.custom}</option></select></label>{#if density === 'custom'}<label>{t.wordsPerPage}<input type="number" min="10" max="1200" bind:value={customWordsPerPage} on:input={markDirty} /></label>{/if}<label>{t.fontSize}<input type="number" min="6" max="48" bind:value={fontSize} on:input={markDirty} /></label></fieldset>
    <fieldset class="test-pdf__panel"><legend>{t.images}</legend><label>{t.imageMode}<select bind:value={imageMode} on:change={markDirty}><option value="none">{t.noImages}</option><option value="placeholder">{t.placeholder}</option><option value="upload">{t.uploaded}</option></select></label>{#if imageMode === 'upload'}<label>{t.imageUpload}<input type="file" accept="image/jpeg,image/png,image/webp" on:change={handleImageInput} /></label>{#if uploadedImageName}<p class="test-pdf__help">{uploadedImageName}</p>{/if}{/if}{#if imageMode !== 'none'}<label>{t.imagePlacement}<select bind:value={imagePlacement} on:change={markDirty}><option value="top">{t.top}</option><option value="bottom">{t.bottom}</option><option value="wrap">{t.wrap}</option><option value="text">{t.textOnly}</option></select></label><label>{t.imageScope}<select bind:value={imageScope} on:change={markDirty}><option value="all">{t.allPages}</option><option value="odd">{t.oddPages}</option><option value="even">{t.evenPages}</option><option value="custom">{t.customPages}</option></select></label>{#if imageScope === 'custom'}<label>{t.customPages}<input bind:value={imagePages} placeholder="1-3,5,8" on:input={markDirty} /></label><p class="test-pdf__help">{t.customPagesHelp}</p>{/if}{/if}</fieldset>
  </div>
  <section class="test-pdf__review" aria-labelledby="summary-title"><div><h3 id="summary-title">{t.summary}</h3><dl><div><dt>{t.pages}</dt><dd>{pageCount}</dd></div><div><dt>{t.pageSize}</dt><dd>{formatPageSize()} · {orientation === 'portrait' ? t.portrait : t.landscape}</dd></div><div><dt>{t.estimatedChars}</dt><dd>{estimatedCharacters.toLocaleString()}</dd></div><div><dt>{t.includesImages}</dt><dd>{includesImages ? `${imagePagesSet.size} ${t.pages}` : t.noImages}</dd></div><div><dt>{t.estimatedSize}</dt><dd>~{estimatedSizeKb} KB</dd></div></dl>{#if isHeavy}<p class="test-pdf__warning">⚠️ {t.warningHeavy}</p>{/if}{#if validationError}<p class="test-pdf__error" role="alert">{validationError}</p>{/if}{#if errorMessage}<p class="test-pdf__error" role="alert">{errorMessage}</p>{/if}{#if statusMessage}<p class="test-pdf__success" role="status">{statusMessage}</p>{/if}<p class="test-pdf__privacy">🔒 {t.privacy}</p></div><div class="test-pdf__actions"><button type="button" class="test-pdf__primary" disabled={isGenerating || Boolean(validationError)} on:click={generatePdf}>{isGenerating ? t.generating : t.generate}</button><button type="button" class="test-pdf__secondary" disabled={isGenerating} on:click={resetConfig}>{t.reset}</button></div></section>
</section>

<PdfResultModal open={isPreviewOpen && Boolean(previewUrl)} pdfUrl={previewUrl} filename={resultFilename} title={t.previewTitle} description={t.previewDescription} closeLabel={t.closePreview} downloadLabel={t.download} openLabel={t.openPreview} on:close={closePreview} />
